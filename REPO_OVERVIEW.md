...
1. **Documentation gap**: README does not yet describe setup or runtime dependencies.
2. **Role consistency**: app supports a `manager` role in code, while migration check constraint currently lists only `citizen` and `staff`.
3. **Security hygiene**: Supabase URL + publishable key are hardcoded in client code; consider environment-variable injection for cleaner config management.
4. **Dependency readiness**: local test command fails without installed node modules in a fresh environment.
